...
def count_std(df, base_column, count_column):
    tp = df.groupby(base_column).agg({count_column: np.std}).reset_index()
...
print(train)
column=[ 'event_id','jet_id']
print(train.columns)
# train.drop('jet_id',axis=1)
...
#     )
#     model.fit(X_train,
#               Y_train, eval_set=(X_valid, Y_valid), use_best_model=True,plot=True
#               )
